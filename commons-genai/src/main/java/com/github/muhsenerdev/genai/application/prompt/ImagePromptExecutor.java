...
public class ImagePromptExecutor implements PromptExecutorStrategy {
    ...
    @Override
    public AiResponse execute(PromptDefinition definition, Map<String, Object> inputVariables)
            throws PromptExecutionException {
        try {
            PromptOutputType outputType = definition.getOutputType();
            if (!supports(outputType))
                throw new RuntimeException("Unsupported output type: " + outputType + ", this strategy requires IMAGE");
            ...
            HttpOptions httpOptions = HttpOptions.builder().apiVersion(apiVersion)
                    .retryOptions(HttpRetryOptions.builder().attempts(1).httpStatusCodes(408, 429)).build();

            try (Client client = Client.builder().apiKey(apiKey).httpOptions(httpOptions).build()) {

                GenerateContentConfig config = GenerateContentConfig.builder().responseModalities("TEXT", "IMAGE")
                        .build();

                GenerateContentResponse response = client.models.generateContent(model, finalUserMessage, config);
            ...
            throw new PromptExecutionException("Failed to execute prompt with slug: %s, due to :%s"
